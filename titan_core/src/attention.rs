...
impl MultiHeadAttention {
    ...
    pub fn forward(&self, x: &Tensor, rope: &RotaryEmbedding) -> Result<Tensor> {
        ...
        // Reshape for multi-head: [T, H, Hd] -> [H, T, Hd]
        let q = q.reshape((t_size, self.num_heads, self.head_dim))?.transpose(0, 1)?;
        let k = k.reshape((t_size, self.num_heads, self.head_dim))?.transpose(0, 1)?;
        let v = v.reshape((t_size, self.num_heads, self.head_dim))?.transpose(0, 1)?;
